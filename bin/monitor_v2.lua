...
local StartStrategy = require('strategy/start_strategy_v2')
local StopStrategy = require('strategy/stop_strategy_v2')
...
local function updateHXcgData(hxcgCell, model)
    local state = model:getState()

    addChartValue(hxcgCell.chart, state.steamLevel.amount)

    hxcgCell.fields.enabled.text = state.enabled and 'ENABLED' or 'DISABLED'
    hxcgCell.fields.enabled.colors.text = state.enabled and 0x00ff00 or 0xff0000

    hxcgCell.fields.steam.text = tostring(state.steamLevel.amount) .. ' mB'
    hxcgCell.fields.steam.colors.text = (state.steamLevel.amount >
            state.steamLevel.capacity / 2) and 0x00ff00 or
            0xff0000

    hxcgCell.fields.water.text = tostring(state.waterLevel.amount) .. ' mB'
    hxcgCell.fields.water.colors.text = (state.waterLevel.amount >
            state.waterLevel.capacity / 2) and
            0x00ff00 or 0xff0000

    hxcgCell.fields.fuel.text = tostring(state.fuelLevel.amount) .. ' mB'
    hxcgCell.fields.fuel.colors.text = (state.fuelLevel.amount >
            state.waterLevel.capacity / 2) and
            0x00ff00 or 0xff0000
end
...
local function updateUiData(statCells, hmodel)
    ...
    updateHXcgData(statCells['HXCG-1'], hmodel.models['HXCG-1'])
    updateHXcgData(statCells['HXCG-2'], hmodel.models['HXCG-2'])
